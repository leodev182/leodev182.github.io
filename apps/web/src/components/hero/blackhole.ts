...
export function initBlackHole(canvas: HTMLCanvasElement): () => void {
  ...
  // ── Core group: black sphere + rim aura ────────────────────────────────────
  ...
  function transition() {
    stateIdx = (stateIdx + 1) % configs.length
    const s = configs[stateIdx]!
  ...
  const stateInterval = setInterval(transition, 10000)
